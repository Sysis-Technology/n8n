import { INodeApp } from 'n8n-workflow';

module.exports = async function (app: INodeApp) {
	// Registrar a extensão
	console.log('🔓 Extensão Enterprise Unlocker carregada');

	try {
		// Obter a instância do License Manager
		const licenseManager = (global as any).licenseMgr;
		...
		if (licenseManager) {
			...
			if (n8nCli && n8nCli.License && n8nCli.License.prototype) {
				...
				n8nCli.License.prototype.init = async function (...args: any[]) {
